...
import { permitsApi, documentsApi, signaturesApi } from '@/lib/api';
            ...
            <main className="flex-1 min-w-0 pb-20">
                ...
                <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-8 sticky top-0 z-30 shadow-sm shadow-slate-100/50">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        {permit.status === 'engineering_review' && (
                            <Button
                                ...
                                onClick={() => {
                                    if (confirm('Approve as Engineering?')) {
                                        permitsApi.approveEngineering(permit.id, 'engineer-id')
                                            .then(() => {
                                    ...
                                    <CardContent className="p-6 space-y-4">
                                        ...
                                        {permit.status === 'draft' && (
                                            <div className="pt-4 border-t border-slate-50 mt-4">
                                                <Button
                                                    className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black h-12 rounded-xl text-xs uppercase tracking-widest"
                                                    disabled={!permit.hazardsIdentified || !permit.controlsRequired || !permit.ppeIdentified || !permit.equipmentIdentified}
                                ...
                                <Card className="border-0 shadow-xl shadow-slate-200/50 bg-white rounded-3xl overflow-hidden">
                                    <CardHeader className="p-6 border-b border-slate-50 bg-slate-50/50">
                                        <CardTitle className="text-[11px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                                            ...
                                            {language === 'tr' ? 'Etkilenen Departman / Eş Zamanlı İş Onayı' : 'Affected Dept / Concurrent Work Approval'}
                                        </CardTitle>
                                    </CardHeader>
                                    <CardContent className="p-6">
                                        {permit.affectedDeptApproved ? (
                                            <div className="p-4 bg-green-50 border border-green-100 rounded-xl flex items-center gap-3 text-green-700">
                                                ...
                                                <span className="font-bold text-sm">{language === 'tr' ? 'Onaylandı: ' : 'Approved by: '} {permit.affectedDeptApprovedBy || 'Manager'}</span>
                                            </div>
                                        ) : (
                                            <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-xl">
                                                <span className="font-bold text-sm text-slate-500">{language === 'tr' ? 'Onay Bekleniyor' : 'Approval Required'}</span>
                                                <Button
                                                    size="sm"
                                        ...
                                        {permit.auditLogs?.map((log: any) => (
                                            <div key={log.id} className="p-5 hover:bg-slate-50/50 transition-colors">
                                                ...
                                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">
                                                    {format(new Date(log.createdAt), 'HH:mm:ss · dd MMM yyyy')}
                                                </p>
                                            </div>
                                        ))}
                                    </div>
