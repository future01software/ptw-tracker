...
export default function NewPermitPage() {
    ...
    const [personnelInput, setPersonnelInput] = useState({ name: '', role: '' });
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const [locationsRes, contractorsRes, userRes] = await Promise.all([
                locationsApi.getAll(),
                contractorsApi.getAll(),
                permitsApi.getStats().catch(() => null) // Just to check if server is up, or we can use seedUser
            ]);

            // Seed a user for development if we don't have authentication yet
            const seedRes = await usersApi.seedUser({
                                                ...
                                                onKeyDown={e => {
                                                    if (e.key === 'Enter') {
                                                        ...
                                                        if (personnelInput.name) {
                                                            setFormData({
                                                                ...formData,
                                                                personnelList: [...formData.personnelList, { name: personnelInput.name, role: 'Worker' }]
                                                            });
                                                            setPersonnelInput({ ...personnelInput, name: '' });
                                                        }
                                                    }
                                                }}
                                            />
